  ...
  subroutine src2_bous(meqn,mbc,mx,my, &
      ...
      use geoclaw_module, only: g => grav, coordinate_system,sea_level
      use bous_module
...
!      Boussinesq type-------------------------------------------------
      ...
      if (coordinate_system == 2) then
          hx = dx*111.32d3*cos((ylower + dy*my/2.)/180.d0*3.1415926d0)
          hy = dy*110.574d3
      endif

     mxy=(mx+2)*(my+2)

     nstep=ceiling(max(dt/hx,dt/hy)*2*sqrt(2.))
     ...
     do ii=1,nstep
          ...
          call read_A(mx,my,meqn,mbc,hx,hy, &
          ...
          CALL DGMRES (2*mxy, psi, XX, NELT, IA, JA, SA, ISYM, MATVEC_TRIAD, &
          ...
          call read_psi(mx,my,meqn,mbc,hx,hy,q0,maux,aux,psi, &
