...
!                                                                     !
!           Jihwan Kim, NGI, Oslo Norway, 2014-2017                   !
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!------------------------------------------------------------------------
         ...
         if (hL-dpL>drytol.or.hR-dpR>drytol) then
             go to 20
         endif
!------------------------------------------------------------------------
         ! Under development
        ...
        !eliminate ghost fluxes for wall
         ...
         do mw=1,3
            ...
            if (abs(fw(1,mw))>1.d-30) then
            ! Not sure on this choice of fwave, but does not make a difference
               if (hL<drytol) then
                  fwave(mu,mw,i)=fw(2,mw)/hR*alpha_1 !/fw(1,mw) !alpha_1
                  fwave(nv,mw,i)=fw(3,mw)/hR*alpha_1 !/fw(1,mw) !alpha_1
               elseif (hR<drytol) then
                  fwave(mu,mw,i)=fw(2,mw)/hL*alpha_1 !/fw(1,mw) !alpha_1
                  fwave(nv,mw,i)=fw(3,mw)/hL*alpha_1 !/fw(1,mw) !alpha_1
               else
                  fwave(mu,mw,i)=fw(2,mw)/(hL+hR)*2.d0*alpha_1 !/fw(1,mw) !alpha_1
                  fwave(nv,mw,i)=fw(3,mw)/(hL+hR)*2.d0*alpha_1 !/fw(1,mw) !alpha_1
               endif
               fwave(mu,mw,i)=fw(2,mw)/(hL+hR)*2.d0*alpha_1 !/fw(1,mw) !alpha_1
               fwave(nv,mw,i)=fw(3,mw)/(hL+hR)*2.d0*alpha_1 !/fw(1,mw) !alpha_1
            endif
