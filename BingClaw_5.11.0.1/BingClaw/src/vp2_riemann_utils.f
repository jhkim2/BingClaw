c-----------------------------------------------------------------------
     ...
     &   hvL,hvR,bL,bR,uL,uR,vL,vR,phiL,phiR,sE1,sE2,drytol,g,sw,fw)
      ...
      ! Modified by Jihwan Kim to use in Bingham model
...
! =========================================================
      subroutine checkval_bing(mx,my,meqn,mbc,
     &                 q,alpha1)
! =========================================================
!
!     Fix unphysical values in H-B model
!     alpha1 <= U/U_p <= 1 must be satisfied.
      ...
      integer, intent(in) :: mbc,mx,my,meqn
      real(kind=8), intent(inout) :: q(meqn,1-mbc:mx+mbc,1-mbc:my+mbc)

      integer  i,j,k
      double precision alpha1,r1
      double precision dp,ds,u,up,h,vp,v
      double precision ubar,upbar,tau_y
         ...
         do j=0,my+1
	       ...
	       u = q(4,i,j)/h
               ...
               if (upbar>1d-8) then
                  ...
                  r1= ubar/upbar
                  dp= (r1-alpha1)*h/(1.d0-alpha1)

                  if (upbar==ubar) dp = h
                  dp = max(0.d0,min(h,dp))
                  ...
                  if (abs(u)<alpha1*abs(up)) then
                     !print*,'u/up<alpha1',i,up,u,u/up
                     up = u/alpha1
                  elseif (abs(u)>abs(up)) then
                     !print*,'u/up>1',i,up,u,u/up
                     up = u
                  endif
				  
                  if (abs(v)<alpha1*abs(vp)) then
                     !print*,'v/vp<alpha1',i,vp,v,v/vp
                     vp = v/alpha1
                  elseif (abs(v)>abs(vp)) then
                     !print*,'v/vp>1',i,vp,v,v/vp
                     vp = v
                  endif

                  if (abs(up)<1.d-8) then
                     up= 0.d0
                     u = 0.d0
                  elseif (abs(vp)<1.d-8) then
                     vp= 0.d0
                     v = 0.d0
                  elseif (abs(u)<1.d-8) then
                     up= 0.d0
                     u = 0.d0
                  elseif (abs(v)<1.d-8) then
                     vp= 0.d0
                     v = 0.d0
                  endif
               else
